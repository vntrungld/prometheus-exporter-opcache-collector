...
class MaxCachedKeysCollectorTest extends TestCase
{
    ...
    public function testRegisterCreatesGaugeWithCorrectValues(): void
    {
        ...
        $statusGetter->shouldReceive('getStatus')
            ->with(null, null)
            ->andReturn(['opcache_statistics' => ['max_cached_keys' => 16229]]);
        $statusGetter->shouldReceive('getStatus')
            ->with('opcache_statistics.max_cached_keys', null)
            ->andReturn(16229);

        /** @var MockInterface|Gauge $gauge */
        $gauge = Mockery::mock(Gauge::class);
        $gauge->shouldReceive('help')->andReturnSelf();
        $gauge->shouldReceive('value')->with(16229)->andReturnSelf();

        /** @var MockInterface|Prometheus $prometheus */
        $prometheus = Mockery::mock(Prometheus::class);
        $prometheus->shouldReceive('addGauge')
            ->with('opcache_max_cached_keys')
            ->once()
            ->andReturn($gauge);

        $collector = new MaxCachedKeysCollector($statusGetter);
