...
class UsedMemoryCollectorTest extends TestCase
{
    ...
    public function testRegisterCreatesGaugeWithCorrectValues(): void
    {
        ...
        $statusGetter->shouldReceive('getStatus')
            ->with(null, null)
            ->andReturn(['memory_usage' => ['used_memory' => 67108864]]);
        $statusGetter->shouldReceive('getStatus')
            ->with('memory_usage.used_memory', null)
            ->andReturn(67108864);
        ...
        $prometheus->shouldReceive('addGauge')
            ->with('opcache_used_memory_bytes')
            ->once()
            ->andReturn($gauge);

        $collector = new UsedMemoryCollector($statusGetter);
