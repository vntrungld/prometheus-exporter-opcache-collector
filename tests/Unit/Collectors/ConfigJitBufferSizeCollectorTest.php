...
class ConfigJitBufferSizeCollectorTest extends TestCase
{
    ...
    public function testRegisterCreatesGaugeWithCorrectValues(): void
    {
        ...
        $statusGetter->shouldReceive('getConfiguration')
            ...
            ->andReturn(['opcache.jit_buffer_size' => 134217728]);
        ...
        $prometheus->shouldReceive('addGauge')
            ->with('opcache_config_jit_buffer_size_bytes')
            ->once()
            ->andReturn($gauge);

        $collector = new ConfigJitBufferSizeCollector($statusGetter);
