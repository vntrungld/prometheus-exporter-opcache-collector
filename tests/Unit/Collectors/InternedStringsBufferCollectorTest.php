...
class InternedStringsBufferCollectorTest extends TestCase
{
    ...
    public function testRegisterCreatesGaugeWithCorrectValues(): void
    {
        ...
        $statusGetter->shouldReceive('getStatus')
            ->with(null, null)
            ->andReturn(['interned_strings_usage' => ['buffer_size' => 8388608]]);
        $statusGetter->shouldReceive('getStatus')
            ->with('interned_strings_usage.buffer_size', null)
            ->andReturn(8388608);

        /** @var MockInterface|Gauge $gauge */
        $gauge = Mockery::mock(Gauge::class);
        $gauge->shouldReceive('help')->andReturnSelf();
        $gauge->shouldReceive('value')->with(8388608)->andReturnSelf();

        /** @var MockInterface|Prometheus $prometheus */
        $prometheus = Mockery::mock(Prometheus::class);
        $prometheus->shouldReceive('addGauge')
            ->with('opcache_interned_strings_buffer_bytes')
            ->once()
            ->andReturn($gauge);

        $collector = new InternedStringsBufferCollector($statusGetter);
